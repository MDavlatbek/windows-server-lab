...
В ходе проверки состояния контроллера домена на базе Server Core подтверждена корректная работа всех инфраструктурных ролей:

![Статус служб Server Core](screenshots/server-core-status.png)

### 2. Получение сетевых настроек клиентом и верификация домена
Клиентская машина `CLIENT-01` успешно авторизовалась в домене `lab.local` и получила сетевую конфигурацию от DHCP-сервера `DC-01`:

![Проверка IP и домена на клиенте](screenshots/client-dhcp-check.png)

### 3. Доступ к общим ресурсам файлового сервера
Проверена доступность сетевых каталогов на сервере `SRV-FILE-01` с учетом разграничения прав доступа NTFS:

![Сетевые папки File Server](screenshots/file-server-share.png)
